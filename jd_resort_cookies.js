...
!(async () => {
    ...
    for (let i = cks.length - 1; i >= 0; i--) {
        const ck = cks[i]
        const rs = await moveJdCookies(ck.id, 100, i)
    ...
    return new Promise((resolve) => {
        ...
        $.get(opts, (error, response, data) => {
            try {
                if (error) {
                    console.log(`${JSON.stringify(error)}`)
                    console.log(`${$.name} getJdCookies API请求失败`)
                    resolve('failed')
                } else {
                    // console.log('jdcookies: ', data)
                    resolve(JSON.parse(data))
                }
            } catch (e) {
                $.logErr(e, response)
...
async function moveJdCookies(envId, fromIndex, toIndex) {
    const opts = {
        url: getMoveJdCookieUrl(envId),
        method: 'put',
        headers: {
            "Authorization": authorization
        },
    ...
    return await axios(opts).then(res => res).catch(error => error)
}

function getMoveJdCookieUrl(envId) {
    return `${domain}/open/envs/${envId}/move?t=${(new Date()).getTime()}`
}
